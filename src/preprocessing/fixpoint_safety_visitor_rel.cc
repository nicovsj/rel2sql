...
FixpointSafetyVisitorRel::FixpointSafetyVisitorRel(RelASTContainer* container,
                                                   const std::string& recursive_relation)
    : SafeVisitorRel(container),
      recursive_relation_(recursive_relation),
      iteration_count_(0) {
  current_placeholder_ = "R0";
}

std::vector<std::string> FixpointSafetyVisitorRel::ExtractHeadVariables(RelBindingsFormula& node) const {
...
BoundSet FixpointSafetyVisitorRel::ExtractPlaceholderSafety(RelFullAppl& node,
...
BoundSet FixpointSafetyVisitorRel::ComputeFixpoint(RelBindingsFormula& node) {
...
void FixpointSafetyVisitorRel::Visit(RelFullAppl& node) {
  ...
  SafeVisitorRel::Visit(node);
}

bool FixpointSafetyVisitorRel::IsPlaceholder(const std::string& id) const {
...
std::string FixpointSafetyVisitorRel::GetNextPlaceholder() {
  return "R" + std::to_string(iteration_count_ + 1);
}

bool FixpointSafetyVisitorRel::BoundSetsEqual(const BoundSet& a, const BoundSet& b) const {
  return a.bounds == b.bounds;
}

BoundSet FixpointSafetyVisitorRel::RemovePlaceholderDomains(const BoundSet& safety) const {
