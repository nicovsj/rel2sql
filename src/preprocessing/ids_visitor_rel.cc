...
void IDsVisitorRel::Visit(RelProgram& node) {
...
void IDsVisitorRel::Visit(RelDef& node) {
  std::string id = node.name;
  if (IsSQLKeyword(id)) {
    throw SemanticException("Relation name '" + id + "' is a reserved SQL keyword",
                            ErrorCode::RESERVED_RELATION_NAME, GetSourceLocationFromNode(&node));
  }
...
void IDsVisitorRel::Visit(RelAbstraction& node) {
  for (auto& expr : node.exprs) {
    if (expr) expr->Accept(*this);
  }
}

void IDsVisitorRel::Visit(RelIDTerm& node) {
  container_->AddVar(node.id);
  deps_.insert(node.id);  // Collect for dependency graph; vars removed in RemoveVarsFromDependencyGraph
}

void IDsVisitorRel::Visit(RelNumTerm& node) {
  RelASTVisitor::Visit(node);
}

void IDsVisitorRel::Visit(RelOpTerm& node) {
  RelASTVisitor::Visit(node);
}

void IDsVisitorRel::Visit(RelParenthesisTerm& node) {
  RelASTVisitor::Visit(node);
}

void IDsVisitorRel::Visit(RelLitExpr& node) {
  RelASTVisitor::Visit(node);
}

void IDsVisitorRel::Visit(RelTermExpr& node) {
  RelASTVisitor::Visit(node);
}

void IDsVisitorRel::Visit(RelProductExpr& node) {
  RelASTVisitor::Visit(node);
}

void IDsVisitorRel::Visit(RelConditionExpr& node) {
  RelASTVisitor::Visit(node);
}

void IDsVisitorRel::Visit(RelAbstractionExpr& node) {
  RelASTVisitor::Visit(node);
}

void IDsVisitorRel::Visit(RelFormulaExpr& node) {
  RelASTVisitor::Visit(node);
}

void IDsVisitorRel::Visit(RelBindingsExpr& node) {
  AddDepsFromBindings(node.bindings);
  RelASTVisitor::Visit(node);
}

void IDsVisitorRel::Visit(RelBindingsFormula& node) {
  AddDepsFromBindings(node.bindings);
  RelASTVisitor::Visit(node);
}

void IDsVisitorRel::Visit(RelPartialAppl& node) {
  AddDepsFromBase(node.base);
  AddDepsFromParams(node.params);
  RelASTVisitor::Visit(node);
}

void IDsVisitorRel::Visit(RelFullAppl& node) {
  AddDepsFromBase(node.base);
  AddDepsFromParams(node.params);
  RelASTVisitor::Visit(node);
}

void IDsVisitorRel::Visit(RelBinOp& node) {
  RelASTVisitor::Visit(node);
}

void IDsVisitorRel::Visit(RelUnOp& node) {
  RelASTVisitor::Visit(node);
}

void IDsVisitorRel::Visit(RelQuantification& node) {
  RelASTVisitor::Visit(node);
}

void IDsVisitorRel::Visit(RelParen& node) {
  RelASTVisitor::Visit(node);
}

void IDsVisitorRel::Visit(RelComparison& node) {
  RelASTVisitor::Visit(node);
}

void IDsVisitorRel::AddDepsFromBase(const std::shared_ptr<RelApplBase>& base) {
...
void IDsVisitorRel::AddDepsFromParams(const std::vector<std::shared_ptr<RelApplParam>>& params) {
  for (const auto& param : params) {
    if (param && param->GetExpr()) param->GetExpr()->Accept(*this);
  }
}

void IDsVisitorRel::AddDepsFromBindings(const std::vector<std::shared_ptr<RelBinding>>& bindings) {
