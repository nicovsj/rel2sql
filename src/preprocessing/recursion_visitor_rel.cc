...
void RecursionVisitorRel::Visit(RelProgram& node) {
  for (auto& def : node.defs) {
    if (def) def->Accept(*this);
  }
}

void RecursionVisitorRel::Visit(RelDef& node) {
  current_q_ = node.name;
  if (node.body) node.body->Accept(*this);
  current_q_.clear();
}

void RecursionVisitorRel::Visit(RelAbstraction& node) {
...
void RecursionVisitorRel::Visit(RelBindingsFormula& node) {
...
void RecursionVisitorRel::Visit(RelBinOp& node) {
  if (node.lhs) node.lhs->Accept(*this);
  if (node.rhs) node.rhs->Accept(*this);
}

void RecursionVisitorRel::Visit(RelQuantification& node) {
  if (node.formula) node.formula->Accept(*this);
}

void RecursionVisitorRel::Visit(RelFullAppl& node) {
...
bool RecursionVisitorRel::IsRecursiveID(const std::string& id) const {
...
bool RecursionVisitorRel::CheckRecursionPattern(const std::shared_ptr<RelFormula>& formula,
...
bool RecursionVisitorRel::IsCallToQ(const RelFullAppl& appl, const std::string& q) const {
