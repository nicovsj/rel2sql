#ifndef REL_AST_REL_AST_BUILDER_H
#define REL_AST_REL_AST_BUILDER_H
...
class RelASTBuilder : public rel_parser::RelParserBaseVisitor {
 public:
  using psr = rel_parser::RelParser;

  RelASTBuilder() = default;

  std::shared_ptr<RelProgram> Build(antlr4::ParserRuleContext* tree);
  std::shared_ptr<RelFormula> BuildFromFormula(antlr4::ParserRuleContext* tree);
  std::shared_ptr<RelExpr> BuildFromExpr(antlr4::ParserRuleContext* tree);
  ...
  // Formulas
  std::any visitFormulaBool(psr::FormulaBoolContext* ctx) override;
 ...
 private:
  static RelCompOp ParseCompOp(const std::string& op);
  static RelTermOp ParseTermOp(const std::string& op);
};

}  // namespace rel2sql

#endif  // REL_AST_REL_AST_BUILDER_H
